...
int main() {
	...
	int size[l] = {
		input_l,
		120,
		8}; /// массив у которого каждый индекс означает кол-во нейронов в слою
	network nn;
	...
	if (to_study) {
		...
		for (int e = 0; ra / double(n) < 1.0; e++) {
			...
			cout << "Right answers: " << ra / n * 100
				 << "% \t Max RA: " << double(maxra) / n * 100 << "(epoch "
				 << maxraepoch << " )"
				 << endl; /// вывод процент максимальных ответов
			if (ra > maxra) {
				maxra = ra;
				maxraepoch = e;
			}
	...
	if (start_universal) {
		...
		cout << "Тест пройден! Процент правильных ответов: "
			 << double(rightAnswers) / double(length) * 100 << " %\n";
	}
	return 0;
}
